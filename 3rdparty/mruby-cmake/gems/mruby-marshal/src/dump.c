...
/* Object links must be keyed by *object identity*, not Ruby `==` semantics.
 * Two distinct objects that compare equal (e.g. two empty arrays, `[] == []`)
 * must never be treated as the same object, otherwise kh_get() (which probes
 * by hash but compares with __hash_equal only) returns false matches and the
 * dump emits bogus TYPE_LINK entries that collapse distinct containers. */
static mrb_bool kh_mrb_value_equal(mrb_state *mrb, mrb_value a, mrb_value b) {
  return mrb_obj_id(a) == mrb_obj_id(b);
}
...
static int iv_count_each(mrb_state *mrb, mrb_sym id, mrb_value value, void *ud) {
  struct iv_count_arg *a = (struct iv_count_arg *)ud;
  /* The encoding ivar ("E") is not serialized as a regular instance variable
   * (it is emitted as the encoding marker), so it must not be counted, just as
   * w_obj_each() skips it when writing the pairs. */
  if (id == a->enc)
    return 0;
  a->count++;
  return 0;
...
static void w_ivar(mrb_state *mrb, mrb_value obj, struct iv_tbl *tbl,
  ...
  // The number of ivars must be written first; load.c's r_ivar() reads it.
  w_long(mrb, num, arg->arg);
  iv_foreach(mrb, tbl, w_obj_each, arg);
}

static void w_objivar(mrb_state *mrb, mrb_value obj,
                      struct dump_call_arg *arg) {
  struct iv_count_arg ca = {0, mrb_intern_lit(mrb, "E")};
  mrb_iv_foreach(mrb, obj, iv_count_each, &ca);
  /* cruby writes the number of instance variables before the (sym, value)
   * pairs, and load.c's r_ivar() reads that count. Without it the stream
   * desyncs and cannot be loaded at all. */
  w_long(mrb, ca.count, arg->arg);
  ...
  if (mrb_nil_p(obj)) {
    ...
    if (arg->regexp_class && mrb_obj_class(mrb, obj) == arg->regexp_class) {
      ...
      case MRB_TT_STRING:
        ...
        /* cruby wraps every string in TYPE_IVAR with an `E` encoding marker
         * (`E => true` for UTF-8, `E => false` for ASCII-8BIT/binary). Without
         * it, non-ASCII strings lose their encoding and the dump no longer
         * matches cruby output. mruby strings are UTF-8 by default; strings
         * created via String#b carry MRB_STR_BINARY. */
        w_byte(mrb, TYPE_IVAR, arg);
        ...
        w_object(mrb,
                 RSTR_BINARY_P(mrb_str_ptr(obj)) ? mrb_false_value()
                                                 : mrb_true_value(),
                 arg, limit);
        break;
